...
#### Phase D: 한국 주식 특이사항 (완료) ✅
1. ✅ **티커 형식**: `005930.KS` (코스피), `000660.KQ` (코스닥) — `.KS` 실패 시 `.KQ` 자동 재시도
2. ✅ **상하한가 ±30% 라인** — 한국 주식 차트에 빨강/파랑 점선
3. ✅ **52주 신고가/신저가 라인** — 모든 주식 차트에 노랑/회색 점선 + 라벨
4. ✅ **Pre/After-market 가격** — Yahoo `preMarketPrice`/`postMarketPrice` 모달 헤더에 표시
5. ✅ **한국 주식 데이터 안내** — Yahoo 정확도 한계 + 네이버/다음 금융 링크
6. ✅ **한국 유니버스 확장** — 40개 → 60개 (KOSPI 50 + KOSDAQ 인기 종목)
7. ⏭️ **VI 발동 이력** — 데이터 소스 부재로 보류
8. ⏭️ **외국인/기관 매매 동향** — 네이버 크롤링 CORS 문제로 보류 (외부 백엔드 필요)
